...
export default async function Page({
  ...
  const allData = await getAllData<{
    ...
    kanji: string;
  }>(`contain-${level}-common-words`, "*", undefined, undefined, {
    limit: questionsCount,
    offset: randomStart,
  });

  // const thousandData = await getAllData<{
  //   kana: {
  //     text: string;
  //   }[];
  //   kanji: string;
  ...
  // console.log(
  //   "sini oi",
  //   thousandData?.filter((k) => k.kana.length > 1),
  // );

  const allQuestionsRandomized = randomizeArray(allData || []);
  console.log(
    allQuestionsRandomized?.reduce((a1, c1) => {
      return [
        ...a1,
        ...c1?.sense?.reduce((a, c) => {
