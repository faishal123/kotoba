...
type LevelType = {
  ...
  href: string;
  description?: string;
  skipChooseQuestionCount?: boolean;
};
...
export const LevelChoice = ({
  ...
  const trigger = customTrigger || (isBreakingXs ? (
    <Card className="flex gap-5 p-5 w-full text-foreground items-center">
      ...
      <div className="flex-1 max-w-[calc(100vw-80px-40px-20px-60px)]">
        {level.name && <div className="font-bold">{level.name}</div>}
  ...
  return (
    <LevelChoice
      ...
      customTrigger={
        <Button
          className={className}
          label={`Take All ${allCharactersLevel.href.includes("katakana")
            ? "Katakana"
            : "Hiragana"
            } Characters Quiz`}
          variant="primary"
  ...
  return (
    <div className="relative flex pb-[calc(40px+126px)] sm:pb-10 p-10 gap-0 sm:gap-10 transition-all">
      <div className="flex flex-col gap-5 w-0 transition-all overflow-hidden sm:w-[250px]">
        {allCharactersLevel && <AllCharactersLevelButton
      ...
      >
        {(isBreakingSm || isBreakingXs) && allCharactersLevel && (
        ...
        {choices?.map((choice) => {
          return <LevelChoice level={choice} key={choice.name || choice.character} />;
        })}
      </Card>
